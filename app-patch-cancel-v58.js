...
export const applyCancelPatch = source => {
  source = replaceBlock(source,
    ...
    String.raw`    const confirmCancelSale = async payload => {
        ...
        const canceledCostAmount = roundMoney(selected.reduce((sum, row) => sum + row.unitCost * row.quantity, 0));
        ...
        if (isDirectSale) {
            ...
            customerRefundAmount = isTotal ? currentContractValue : roundMoney(currentContractValue * fraction);
            storeImpactAmount = isTotal ? storeNetBeforeCancellation : roundMoney(storeNetBeforeCancellation * fraction);
            remainingContractValue = roundMoney(Math.max(0, currentContractValue - customerRefundAmount));
        } else {
            effectivePaidBeforeCancellation = roundMoney(Math.max(0, receivedOnTermSale - priorCustomerRefunds));
            remainingContractValue = isTotal ? 0 : roundMoney(currentContractValue * (1 - fraction));
            customerRefundAmount = isTotal ? effectivePaidBeforeCancellation : roundMoney(Math.max(0, effectivePaidBeforeCancellation - remainingContractValue));
            storeImpactAmount = customerRefundAmount;
            remainingToPay = isTotal ? 0 : roundMoney(Math.max(0, remainingContractValue - effectivePaidBeforeCancellation));
            storeNetBeforeCancellation = effectivePaidBeforeCancellation;
        }

        const canceledContractValue = roundMoney(Math.max(0, currentContractValue - remainingContractValue));
        const profitImpactAmount = roundMoney((isDirectSale ? storeImpactAmount : canceledContractValue) - canceledCostAmount);
        const customerAllocations = isDirectSale ? allocateMoney(customerRefundAmount, selected) : [];
        const storeAllocations = isDirectSale ? allocateMoney(storeImpactAmount, selected) : [];
        const contractAllocations = allocateMoney(canceledContractValue, selected);

        const event = {
            id: 'sale-cancel-' + Date.now(), type: isTotal ? 'total' : 'partial', date: payload.date, createdAt: new Date().toISOString(),
            reason: payload.reason, fraction, canceledContractValue, canceledCostAmount, profitImpactAmount,
            customerRefundAmount, storeImpactAmount, refundAmount: storeImpactAmount,
            remainingContractValue, remainingToPay, effectivePaidBeforeCancellation, storeNetBeforeCancellation,
            paymentMethod: sale.paymentMethod || null, isCardCancellation: isCardSale, feeResponsibility,
            ...
            items: selected.map((row, index) => {
                ...
                return {
                    ...
                    ...(isDirectSale ? { customerPaidAmount: roundMoney(customerAllocations[index]), storeNetAmount: roundMoney(storeAllocations[index]) } : {})
                };
            })
        };
        ...
        if (isTotal) {
            ...
            if (!isDirectSale && Array.isArray(sale.installments)) {
            ...
            updateData.status = isDirectSale ? 'completed' : remainingToPay > 0.004 ? 'active' : 'completed';
