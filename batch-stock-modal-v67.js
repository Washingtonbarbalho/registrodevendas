...
export const BatchStockModal = ({ isOpen, onClose, products = [], userId, onSuccess }) => {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setType('compra'); setSearch(''); setRows([]); setNotes(''); setPaymentMethod('pix'); setPaymentDueDate(''); setSaving(false);
  }, [isOpen]);
  ...
  const handleTypeChange = value => {
    setType(value);
    if (value !== 'compra') { setPaymentMethod('pix'); setPaymentDueDate(''); }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (isDeferred && !paymentDueDate) return alert('Informe a data de vencimento da compra.');
    ...
    try {
      ...
      onSuccess?.({ batchId, type, itemCount: rows.length, total: batchTotal, items: computed });
      onClose?.();
    } catch (error) {
    ...
    h('div', { className: 'batch67-panel' },
      ...
      h('div', { className: 'batch67-body' },
        ...
        h('section', { className: 'batch67-card' },
          ...
          rows.length === 0 ? h('div', { className: 'batch67-empty' }, 'Busque e adicione os produtos que serão movimentados.') : h('div', { className: 'batch67-selected-list' }, rows.map(row => h('div', { key: row.productId, className: 'batch67-selected-row' },
        ...
        isPurchase && h('section', { className: 'batch67-card batch67-payment' },
          h('div', { className: 'batch67-payment-title' }, h(CreditCard, { size: 18 }), h('div', null, h('strong', null, 'Pagamento da compra'), h('span', null, 'O Financeiro receberá um único lançamento pelo total deste lote.'))),
          h('label', { className: 'batch67-field' }, h('span', null, 'Forma de pagamento'), h('select', { value: paymentMethod, onChange: e => { setPaymentMethod(e.target.value); if (!['credit', 'term'].includes(e.target.value)) setPaymentDueDate(''); } }, PAYMENT_OPTIONS.map(option => h('option', { key: option.value, value: option.value }, option.label)))),
          isDeferred && h('label', { className: 'batch67-field' }, h('span', null, paymentMethod === 'credit' ? 'Vencimento do cartão' : 'Vencimento da compra'), h('div', { className: 'batch67-date' }, h(CalendarDays, { size: 17 }), h('input', { type: 'date', min: getBrazilDateString(), value: paymentDueDate, onChange: e => setPaymentDueDate(e.target.value) }))),
          h('div', { className: 'batch67-total' }, h('span', null, isDeferred ? 'Conta a pagar' : 'Saída financeira'), h('strong', null, formatCurrency(batchTotal)))
        ),
