...
export const SaleCancellationModal = ({ isOpen, sale, onClose, onConfirm }) => {
  ...
  const selected = useMemo(() => (sale?.items || []).map((item, index) => {
    ...
    return { index, item, available, quantity, amount: money(quantity * num(item.price)) };
  }).filter(row => row.quantity > 0), [sale, mode, quantities]);

  const selectedAmount = selected.reduce((sum, row) => sum + row.amount, 0);
  const activeGoodsValue = (sale?.items || []).reduce((sum, item) => sum + num(item.price) * num(item.quantity), 0);
      ...
      h('div', { className: 'sale47-scroll' },
        ...
        h('section', { className: 'sale47-card' },
          h('div', { className: 'sale47-card-title' },
            h('div', null, h('strong', null, mode === 'total' ? 'Todos os produtos serão cancelados' : 'Escolha os produtos e quantidades'), h('span', null, mode === 'total' ? 'O estoque será devolvido integralmente.' : 'Somente as quantidades selecionadas voltarão ao estoque.'))
          ),
            ...
            (sale.items || []).map((item, index) => {
              ...
              return h('div', { key: `${item.productId || 'item'}-${index}`, className: 'sale47-item' },
                h('div', { className: 'sale47-item-copy' },
                  ...
                  h('span', null, `${available} un. disponíveis para cancelamento · ${formatCurrency(item.price || 0)} cada`)
                ),
                mode === 'total'
