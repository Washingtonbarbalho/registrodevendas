...
export const applyFinalPatches = source => {
  ...
  const freshModules = new Set([
    ...
    'aba-relatorios-v62','reports-engine-v62','aba-taxas','payment-settings','utils'
  ]);
  ...
  source = source.replace(/(['"])(\.\/[^'"]+?\.js)(?:\?[^'"]*)?\1/g, (match, quote, modulePath) => {
    ...
    if (freshModules.has(moduleName) || resolved.includes('v52') || resolved.includes('v54') || resolved.includes('v59') || resolved.includes('v60') || resolved.includes('v62')) url.search = '?v=62';
    return `'${url.href}'`;
  });
