...
import {
    Search, Phone, FileText, MapPin, ShieldCheck, Plus, Users, X, Lock,
    ChevronRight
} from 'https://esm.sh/lucide-react@0.292.0';
import { formatCurrency, analyzeCustomerCredit, maskMoney, parseMoney } from './utils.js?v=21';
import { Pagination, MoneyInput } from './components.js?v=21';
import { CustomerDetailsModal } from './modals-fixed.js?v=21';
import { db, auth, APP_ID } from './firebase-config.js?v=21';
import { doc, updateDoc } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-firestore.js";

export const AbaClientes = ({
    customerSearch,
    setCustomerSearch,
    ...
    const [detailsModal, setDetailsModal] = useState({ open: false, customer: null });
    ...
    const openCreditSettings = customer => {
        const hasManualLimit = customer.creditLimit !== undefined
        ...
        setManualLimit(hasManualLimit
            ? maskMoney(Math.round((Number(customer.creditLimit) || 0) * 100))
            : ''
        );
    ...
    const saveCreditSettings = async () => {
        ...
        try {
            await updateDoc(
                doc(db, 'artifacts', APP_ID, 'users', auth.currentUser.uid, 'customers', customer.id),
                {
                    creditEnabled,
    ...
    const selectedCreditInfo = creditModal.customer
        ? analyzeCustomerCredit(creditModal.customer, 0, sales)
        : null;

    const pendingLimit = limitMode === 'manual'
        ? parseMoney(manualLimit)
        : selectedCreditInfo?.automaticLimit || 0;

    const pendingAvailable = creditEnabled
        ? Math.max(0, pendingLimit - (selectedCreditInfo?.currentDebt || 0))
        : 0;
            ...
            React.createElement('div', {
                ...
                React.createElement('div', { className: "credit-settings-modal-panel app-modal-panel desktop-modal desktop-modal-credit bg-white rounded-2xl w-full max-w-md shadow-2xl animate-fade-in flex flex-col max-h-[92vh]" },
                    ...
                    React.createElement('div', { className: "desktop-modal-body credit-settings-grid flex-1 overflow-y-auto p-4 space-y-4" },
                        React.createElement('div', { className: `credit-status-card desktop-span-full p-4 rounded-xl border ${creditEnabled ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-200'}` },
                            React.createElement('div', { className: "flex items-center justify-between gap-4" },
                                React.createElement('div', null,
                                    React.createElement('p', { className: `font-bold text-sm ${creditEnabled ? 'text-emerald-800' : 'text-slate-600'}` },
                                        creditEnabled ? "Cliente ativo para compras a prazo" : "Cliente inativo para compras a prazo"
                                    ),
                                    React.createElement('p', { className: "text-xs text-slate-500 mt-1" },
                                        creditEnabled
                                            ? "Novas vendas serão avaliadas pelo limite configurado."
                                            : "O sistema recusará novas compras a prazo para este cliente."
                                    )
                                ),
                                React.createElement('button', {
                                    ...
                                    React.createElement(creditEnabled ? ShieldCheck : Lock, { size: 15 }),
                                    creditEnabled ? "Ativo" : "Inativo"
                                )
                            )
                        ),
                        React.createElement('div', { className: "credit-limit-card bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3" },
                            ...
                            React.createElement('div', { className: "grid grid-cols-2 gap-2" },
                                React.createElement('button', {
                                    onClick: () => setLimitMode('automatic'),
                                    className: `p-3 rounded-xl border text-xs font-bold ${limitMode === 'automatic' ? 'border-yellow-400 bg-yellow-50 text-yellow-800' : 'border-slate-200 bg-white text-slate-500'}`
                                }, "Automático"),
                                React.createElement('button', {
                                    onClick: () => setLimitMode('manual'),
                                    className: `p-3 rounded-xl border text-xs font-bold ${limitMode === 'manual' ? 'border-yellow-400 bg-yellow-50 text-yellow-800' : 'border-slate-200 bg-white text-slate-500'}`
                                }, "Personalizado")
                            ),
                            limitMode === 'manual'
                                ? React.createElement('div', null,
                                    ...
                                    React.createElement(MoneyInput, {
                                        ...
                                        className: "w-full p-3 pl-9 border border-slate-200 rounded-xl font-bold"
                                    })
                                )
                            ...
                            React.createElement('label', { className: "flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer" },
                                React.createElement('input', {
                                    ...
                                    onChange: event => setIgnoreOverdue(event.target.checked),
                    ...
                    React.createElement('div', { className: "desktop-modal-footer p-4 border-t border-slate-100 flex gap-3" },
                        React.createElement('button', {
                            onClick: closeCreditSettings,
                            className: "flex-1 p-3 bg-slate-100 text-slate-600 font-bold rounded-xl"
                        }, "Cancelar"),
                        React.createElement('button', {
                            onClick: saveCreditSettings,
                            disabled: savingCredit,
                            className: "flex-1 p-3 bg-slate-900 text-white font-bold rounded-xl disabled:opacity-50"
                        }, savingCredit ? "Salvando..." : "Salvar crédito")
                    )
                )
            ),
            document.body
    ...
    return React.createElement(React.Fragment, null,
        React.createElement('section', { className: "page-stack animate-fade-in" },
            React.createElement('div', { className: "page-heading" },
                React.createElement('div', { className: "page-heading-copy" },
                    ...
                    React.createElement('p', { className: "page-description" }, "Clique em um cliente para consultar cadastro, crédito e histórico de compras.")
                ),
                React.createElement('button', {
            ...
            React.createElement('div', { className: "toolbar" },
                ...
                React.createElement('span', { className: "result-count" },
                    `${sortedCustomers.length} ${sortedCustomers.length === 1 ? 'cliente' : 'clientes'}`
                )
            ),

            React.createElement('div', { className: "list-shell customer-list-shell" },
                React.createElement('div', { className: "list-header md:grid-cols-[minmax(0,1.7fr)_minmax(150px,1fr)_130px_160px_34px]" },
                    React.createElement('span', null, "Cliente"),
                    React.createElement('span', null, "Localização"),
                    React.createElement('span', null, "Em aberto"),
                    React.createElement('span', null, "Crédito disponível"),
                    React.createElement('span', null, "")
                ),
                ...
                paginatedCustomers.length === 0
                    ...
                    : paginatedCustomers.map(customer => {
                        ...
                        const availableClass = inactive
                            ? 'text-slate-400'
                            : creditInfo.availableLimit > 0
                                ? 'text-emerald-700'
                                : 'text-red-600';
                        ...
                        return React.createElement('div', {
                            ...
                            className: "list-row customer-list-row grid-cols-[minmax(0,1fr)_auto] md:grid-cols-[minmax(0,1.7fr)_minmax(150px,1fr)_130px_160px_34px]"
                        },
                            React.createElement('div', { className: "list-main" },
                                ...
                                React.createElement('div', { className: "flex flex-wrap items-center gap-x-3 gap-y-1 mt-1" },
                                    customer.phone && React.createElement('span', { className: "list-meta flex items-center gap-1" },
                                        React.createElement(Phone, { size: 12 }),
                                        customer.phone
                                    ),
                                    customer.document && React.createElement('span', { className: "list-meta flex items-center gap-1" },
                                ...
                                React.createElement('div', { className: "md:hidden mt-3 flex flex-wrap gap-2" },
                                    ...
                                    inactive
                                        ...
                                        : React.createElement('span', {
                                            className: `status-badge ${creditInfo.availableLimit > 0 ? 'status-paid' : 'status-canceled'}`
                                        }, `Disponível: ${formatCurrency(creditInfo.availableLimit)}`),
                                    React.createElement('span', { className: "status-badge status-info" },
                                        creditInfo.limitSource === 'manual' ? 'Limite personalizado' : 'Limite automático'
                                    )
                                )
                            ),
                            React.createElement('div', { className: "hidden md:block list-meta" },
                                customer.cityState
                                    ? React.createElement('span', { className: "flex items-center gap-1.5" },
                                        React.createElement(MapPin, { size: 14, className: "text-slate-400" }),
                                        customer.cityState
                                    )
                            ...
                            React.createElement('div', { className: "hidden md:block" },
                                React.createElement('span', {
                                    className: `text-xs font-extrabold ${creditInfo.currentDebt > 0 ? 'text-orange-600' : 'text-slate-400'}`
                                }, formatCurrency(creditInfo.currentDebt))
                            ),
                            React.createElement('div', { className: "hidden md:flex items-center gap-1.5" },
                                ...
                                React.createElement('div', null,
                                    React.createElement('span', { className: `text-xs font-extrabold block ${availableClass}` },
                                        inactive ? 'Inativo' : formatCurrency(creditInfo.availableLimit)
                                    ),
                                    !inactive && React.createElement('span', { className: "text-[9px] font-bold text-slate-400 uppercase" },
                                        creditInfo.limitSource === 'manual' ? 'Personalizado' : 'Automático'
                                    )
                                )
                            ),
                            React.createElement('div', { className: "grid place-items-center text-slate-300" },
        ...
        React.createElement(CustomerDetailsModal, {
            ...
            onEdit: customer => {
                ...
                setCustomerModalData({ open: true, data: customer });
            },
            onCredit: customer => {
                closeCustomerDetails();
                openCreditSettings(customer);
            },
            onDelete: customer => {
                closeCustomerDetails();
                requestDelete('customer', customer.id);
            }
        }),
