...
export const applyReportsPatch = source => {
  ...
  source = replaceRequired(source, financeImport, financeImport + "\nimport { AbaRelatorios } from './aba-relatorios-v65.js';", 'o módulo dos Relatórios');

  const ratesNav = "        { id: 'rates', label: 'Taxas e juros', shortLabel: 'Taxas', icon: BadgePercent }";
  source = replaceRequired(source, ratesNav, "        { id: 'reports', label: 'Relatórios', shortLabel: 'Relat.', icon: LayoutDashboard },\n" + ratesNav, 'a navegação dos Relatórios');

  const financeRender = "                    : view === 'finance' ? React.createElement(AbaFinanceiro, {";
  source = replaceRequired(source, financeRender, `                    : view === 'reports' ? React.createElement(AbaRelatorios, {
                        userId: user.uid,
                        sales,
                        products,
                        customers,
