...
const INTEGRATED_SYSTEMS = [
    { id: 'main', label: 'Sistema Principal', shortLabel: 'Principal', description: 'Cadastros, vendas, cobranças e catálogo.' },
    { id: 'crm', label: 'CRM Aura', shortLabel: 'CRM', description: 'Pós-venda, fidelidade e relacionamento.', src: './crmaura.html' },
    { id: 'admin', label: 'ADM Aura', shortLabel: 'ADM', description: 'Painel administrativo e gestão operacional.', src: './admaura.html' }
];
...
const getSystemAccentClasses = (systemId, active) => {
    if (active) return 'bg-white text-slate-900 border-white shadow-sm';
    return 'bg-transparent text-slate-300 border-slate-700 hover:bg-slate-800/80 hover:text-white';
};

const getSystemIconToneClasses = (systemId, active) => {
    if (active) {
        if (systemId === 'crm') return 'text-blue-500';
        if (systemId === 'admin') return 'text-emerald-500';
        return 'text-yellow-500';
    }
    if (systemId === 'crm') return 'text-blue-300';
    if (systemId === 'admin') return 'text-emerald-300';
...
const IntegratedSystemFrame = ({ system, frameVersion, onBackToMain, userProfile }) => {
    ...
    return React.createElement('div', { className: "animate-fade-in" },
        React.createElement('div', { className: "bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden" },
            React.createElement('div', { className: "flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 border-b border-slate-100 bg-slate-50/70" },
                React.createElement('div', { className: "flex items-center gap-3 min-w-0" },
                    React.createElement('div', { className: `w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${system.id === 'crm' ? 'bg-blue-50 text-blue-500' : 'bg-emerald-50 text-emerald-500'}` },
                        React.createElement(SystemIcon, { size: 18 })
                    ),
                    React.createElement('div', { className: "min-w-0" },
                        React.createElement('p', { className: "text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400" }, "Módulo ativo"),
                        React.createElement('h2', { className: "font-bold text-slate-800 truncate" }, system.label),
                        React.createElement('p', { className: "text-xs text-slate-400 truncate" }, userProfile?.storeName || 'Minha Hinode')
                    )
                ),
                React.createElement('div', { className: "flex gap-2 flex-wrap sm:justify-end" },
                    React.createElement('button', { onClick: onBackToMain, className: "px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-600 font-bold hover:bg-slate-100 text-sm" }, "Voltar"),
                    React.createElement('button', { onClick: () => window.open(system.src, '_blank', 'noopener,noreferrer'), className: "px-3 py-2 rounded-xl bg-slate-900 text-white font-bold hover:bg-slate-800 text-sm" }, "Abrir fora")
                )
            ),
                ...
                React.createElement('div', { className: "grid grid-cols-3 gap-2 mb-2" },
                    INTEGRATED_SYSTEMS.map((system) => {
                        ...
                        return React.createElement('button', {
                            ...
                            className: `flex items-center justify-center gap-2 px-3 py-2.5 rounded-2xl border transition-all min-w-0 ${getSystemAccentClasses(system.id, active)}`
                        },
                            React.createElement(Icon, { size: 15, className: getSystemIconToneClasses(system.id, active) }),
                            React.createElement('span', { className: "font-bold text-sm truncate" }, system.shortLabel)
                        );
                    })
                ),
                systemView === 'main'
                    ...
                    : React.createElement('div', { className: "flex items-center justify-between text-xs text-slate-400 px-1 pt-1" },
                        React.createElement('span', null, activeIntegratedSystem.label),
                        React.createElement('button', { onClick: () => setExternalFrameVersion(prev => prev + 1), className: "flex items-center gap-1 text-slate-300 hover:text-white font-medium" }, React.createElement(RefreshCw, { size: 13 }), 'Recarregar')
                    )
            )
        ),
