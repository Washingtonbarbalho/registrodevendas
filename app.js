...
const Dashboard = ({ user, userProfile, onLogout }) => {
    ...
    const allFinancialTransactions = useMemo(() => {
        ...
        // 1. Vendas Diretas e Taxas (Tirado o filtro de cancelado, se entrou no caixa, fica no histórico)
        sales.filter(s => s.saleType === 'direct').forEach(s => {
        ...
        // 2. Entradas e Parcelas de Vendas a Prazo (Também tirado o filtro, se pagou, entrou!)
        sales.filter(s => (s.saleType === 'prazo' || !s.saleType)).forEach(s => {
        ...
        // 4. Estornos (Vendas Canceladas) - Aqui é onde o caixa balanceia!
        sales.filter(s => s.status === 'canceled').forEach(s => {
            ...
            // Para vendas diretas canceladas, estorna o valor líquido
            if (s.saleType === 'direct') {
                let liquidValue = s.totalPrice;
                if (s.feeConfig && s.feeConfig.type === 'sem_juros') liquidValue -= (s.feeConfig.value || 0);
                refunded = liquidValue;
            }
    ...
    const dashboardTotals = useMemo(() => {
        // Vendas válidas são usadas apenas para as projeções de lucro estimado e o que falta receber
        ...
        // Soma baseada no período escolhido verificando a lista mestra de finanças (Inclui as entradas das canceladas e os estornos)
