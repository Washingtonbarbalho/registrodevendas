...
import { 
    ...
    ConfirmModal, WhatsAppChooserModal, ProductModal, StockMovementModal, TransactionModal
} from './modals.js';
...
import { AbaFinanceiro } from './aba-financeiro.js';
...
const Dashboard = ({ user, userProfile, onLogout }) => {
    ...
    // Lógica Financeira (Cérebro da Aba Financeiro)
    const allFinancialTransactions = useMemo(() => {
        ...
        sales.filter(s => s.saleType === 'direct' && s.status !== 'canceled').forEach(s => {
            ...
            if (s.feeConfig && s.feeConfig.type === 'sem_juros') {
                liquidValue -= (s.feeConfig.value || 0);
                feeNote = ` (Desconto Maquininha: ${formatCurrency(s.feeConfig.value)})`;
            }
            list.push({
                id: `v-dir-${s.id}`, date: s.saleDate + 'T12:00:00.000Z', 
                description: `Venda à Vista - ${s.customerName}${feeNote}`, 
                type: 'income', category: 'Venda Caixa', amount: liquidValue, source: 'system'
            });
        });

        // 2. Vendas a Prazo (Entradas e Pagamentos de Parcelas)
        sales.filter(s => (s.saleType === 'prazo' || !s.saleType) && s.status !== 'canceled').forEach(s => {
            if (s.entryAmount > 0) {
                list.push({
                    id: `v-ent-${s.id}`, date: s.saleDate + 'T12:00:00.000Z',
                    description: `Entrada Venda a Prazo - ${s.customerName}`,
                    type: 'income', category: 'Entrada Prazo', amount: s.entryAmount, source: 'system'
                });
            }
            if (s.installments) {
                s.installments.forEach((inst, idx) => {
                    if (inst.history && inst.history.length > 0) {
                        inst.history.forEach((h, hIdx) => {
                            if (h.type !== 'abatement') {
                                list.push({
                                    id: `v-parc-${s.id}-${idx}-${hIdx}`, date: h.date,
                                    description: `Pagto Parcela ${inst.number} - ${s.customerName}`,
                                    type: 'income', category: 'Recebimento', amount: h.amount, source: 'system'
                                });
                            }
                        });
                    } else if (inst.paid && inst.paidAt) {
                        list.push({
                            id: `v-parc-${s.id}-${idx}`, date: inst.paidAt,
                            description: `Pagto Parcela ${inst.number} - ${s.customerName}`,
                            type: 'income', category: 'Recebimento', amount: inst.amount, source: 'system'
                        });
                    }
                });
            }
        ...
        products.forEach(p => {
            if (p.movements) {
                p.movements.forEach(m => {
                    if (m.type === 'compra' && m.unitCost > 0) {
                        list.push({
                            id: `compra-${p.id}-${m.id}`, date: m.date,
                            description: `Compra Estoque: ${p.name} (${m.quantity} un)`,
                            type: 'expense', category: 'Mercadoria', amount: m.quantity * m.unitCost, source: 'system'
                        });
                    }
                });
            }
        ...
        manualTransactions.forEach(t => {
            list.push({
                ...
                type: t.type, category: t.category, amount: t.amount, source: 'manual'
            });
        });
    ...
    const dashboardTotals = useMemo(() => {
        ...
        validSales.forEach(s => {
            if (s.installments) {
                s.installments.forEach((i, idx) => {
                    ...
                    if (i.history) {
                        i.history.forEach(h => { if (h.type !== 'abatement' && h.date >= dashStartDate && h.date <= dashEndDate) cashIn += h.amount; });
                    }
                    if (!i.paid) {
        ...
        return { totalReceivable, totalReceived: cashIn, totalOverdue, totalUpcoming, estimatedProfit, realProfit, periodCost, overdueList, upcomingList };
    ...
    const handleOpenWA = (type, sale, installment, historyItem) => {
        // [MESMA LÓGICA DO SEU WHATSAPP MANTIDA AQUI DENTRO...]
        if (!sale) return;
    ...
    return React.createElement('div', { className: "min-h-screen bg-slate-50 pb-24 font-sans text-slate-800" },
        React.createElement('header', { className: "bg-slate-900 text-white p-4 lg:p-6 rounded-b-3xl shadow-lg sticky top-0 z-40 w-full" },
            React.createElement('div', { className: "max-w-7xl mx-auto" },
                ...
                React.createElement('div', { className: "flex space-x-1 overflow-x-auto no-scrollbar justify-start lg:justify-center" },
                    ['dashboard', 'financeiro', 'sales', 'cashier', 'products', 'customers'].map((v) => (
                        React.createElement('button', { key: v, onClick: () => setView(v), className: `pb-2 px-3 lg:px-6 whitespace-nowrap font-medium text-sm lg:text-base transition-colors flex items-center gap-1 ${view === v ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-slate-400 hover:text-white'}` }, 
                            ...
                            v === 'financeiro' ? React.createElement(React.Fragment, null, React.createElement(DollarSign, { size: 16 }), 'Financeiro') : 
        ...
        React.createElement('main', { className: "p-4 max-w-7xl mx-auto" },
            ...
            view === 'dashboard' && React.createElement(AbaVisaoGeral, { dashPeriod, dashStartDate, dashEndDate, setDashPeriod, setDashStartDate, setDashEndDate, dashboardTotals, setInstallmentListModal }),
            view === 'financeiro' && React.createElement(AbaFinanceiro, { allTransactions: allFinancialTransactions, onAddTransaction: () => setTransactionModalOpen(true) }),
