...
import { NewSaleScreen } from './nova-venda.js?v=4';
...
const Dashboard = ({ user, userProfile, onLogout }) => {
    ...
    const dashboardTotals = useMemo(() => {
        ...
        const isDateInPeriod = (dateStr) => {
            ...
            return cleanDate >= dashStartDate && cleanDate <= dashEndDate;
        };
        ...
        const getHistoryReceivedAmount = (historyItem) => {
            if (!historyItem || historyItem.type === 'abatement') return 0;
            let amount = historyItem.amount || 0;
            if (historyItem.type === 'full_surplus') amount += historyItem.surplus || 0;
            return amount;
        };
        const getPrazoReceipts = (sale) => {
            const receipts = [];

            if (sale.entryAmount > 0) {
                receipts.push({
                    date: sale.saleDate,
                    amount: sale.entryAmount,
                    order: `${sale.saleDate || ''}-entrada`
                });
            }

            (sale.installments || []).forEach((installment, idx) => {
                if (installment.history && installment.history.length > 0) {
                    installment.history.forEach((historyItem, historyIndex) => {
                        const receivedAmount = getHistoryReceivedAmount(historyItem);
                        if (receivedAmount > 0) {
                            receipts.push({
                                date: historyItem.date || historyItem.timestamp || installment.paidAt,
                                amount: receivedAmount,
                                order: `${historyItem.date || ''}-${historyItem.timestamp || ''}-${idx}-${historyIndex}`
                            });
                        }
                    });
                    ...
                    receipts.push({
                        ...
                        amount: installment.originalAmount || installment.amount || 0,
                        order: `${installment.paidAt || ''}-${idx}`
                    });
                }
            });

            return receipts
                .filter(receipt => receipt.amount > 0 && receipt.date)
                .sort((a, b) => a.date.localeCompare(b.date) || a.order.localeCompare(b.order));
        };
        const calculatePrazoRealProfitForPeriod = (sale) => {
            const saleCostCents = toCents(sale.totalCost || 0);
            const saleProfitLimitCents = Math.max(0, toCents((sale.totalPrice || 0) - (sale.totalCost || 0)));
            let recoveredCostCents = 0;
            let accumulatedProfitCents = 0;
            let periodProfitCents = 0;

            getPrazoReceipts(sale).forEach(receipt => {
                const receivedCents = toCents(receipt.amount);
                const costStillOpenCents = Math.max(0, saleCostCents - recoveredCostCents);
                const costPartCents = Math.min(receivedCents, costStillOpenCents);
                const profitPartCents = Math.max(0, receivedCents - costPartCents);
                const allowedProfitCents = Math.max(0, saleProfitLimitCents - accumulatedProfitCents);
                const recognizedProfitCents = Math.min(profitPartCents, allowedProfitCents);

                recoveredCostCents += costPartCents;
                accumulatedProfitCents += recognizedProfitCents;

                if (isDateInPeriod(receipt.date)) {
                    periodProfitCents += recognizedProfitCents;
                }
            });

            return fromCents(periodProfitCents);
        };
        
        const totalReceivable = validSales
            .filter(s => isPrazoSale(s))
            .reduce((acc, s) => acc + (s.installments || []).filter(i => !i.paid).reduce((sum, i) => sum + i.amount, 0), 0);
        ...
        validSales.forEach(s => {
            if (s.saleType === 'direct') {
                if (isDateInPeriod(s.saleDate)) {
                    const netDirect = getDirectNetAmount(s);
                    cashIn += netDirect;
            ...
            if (isPrazoSale(s)) {
                getPrazoReceipts(s).forEach(receipt => {
                    if (isDateInPeriod(receipt.date)) {
                        cashIn += receipt.amount;
                    }
                });
                realProfit += calculatePrazoRealProfitForPeriod(s);
            }
        });
        ...
        return { 
            ...
            estimatedProfit, realProfit, overdueList, upcomingList 
        };
    ...
    return React.createElement('div', { className: "app-shell" },
        ...
        React.createElement('nav', { className: "mobile-bottom-nav", 'aria-label': "Navegação principal" },
            navItems.map(item => React.createElement('button', {
                key: item.id,
                onClick: () => setView(item.id),
                className: `mobile-nav-button ${view === item.id ? 'is-active' : ''}`
