...
import { doc, onSnapshot, serverTimestamp, setDoc, updateDoc, writeBatch } from 'https://www.gstatic.com/firebasejs/12.7.0/firebase-firestore.js';
...
const getPurchaseCancellationEvents = movement => {
  ...
  return [{
    id: 'legacy-full', quantity, amount: money(quantity * num(movement.unitCost)),
...
const purchaseItemInfo = (product, movement) => {
  ...
  const remainingAmount = money(remainingQuantity * unitCost);
  const canceledBeforePayment = money(events
    .filter(event => event.hadCashOut === false)
    .reduce((sum, event) => sum + (num(event.amount) || num(event.quantity) * unitCost), 0));
  return { product, movement, originalQuantity, unitCost, originalAmount, events, canceledQuantity, remainingQuantity, remainingAmount, canceledBeforePayment };
};
...
const getPurchaseGroups = products => {
  ...
  return [...groups.values()].map(group => {
    ...
    const remainingAmount = money(group.items.reduce((sum, item) => sum + item.remainingAmount, 0));
    const canceledBeforePayment = money(group.items.reduce((sum, item) => sum + item.canceledBeforePayment, 0));
    const amountActuallyPaid = money(Math.max(0, originalAmount - canceledBeforePayment));
    const financialPaid = deferred ? group.items.every(item => !!item.movement.financialPaid) : true;
    const fullyCanceled = group.items.every(item => item.remainingQuantity <= 0);
    const partiallyCanceled = !fullyCanceled && group.items.some(item => item.canceledQuantity > 0);
    const paidAtItem = group.items.find(item => item.movement.financialPaidAt) || first;
    const purchaseDate = cleanDate(first?.movement?.date);
    const purchaseDateTime = first?.movement?.date || '';
    return {
      ...group, first, paymentMethod, deferred, originalAmount, remainingAmount, canceledBeforePayment,
      amountActuallyPaid, financialPaid, fullyCanceled, partiallyCanceled,
      dueDate: cleanDate(first?.movement?.paymentDueDate),
      paidAt: cleanDate(paidAtItem?.movement?.financialPaidAt),
      paidAtDateTime: paidAtItem?.movement?.financialPaidAtDateTime || '',
...
const FormModal = ({ open, kind, initial, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (!open) return;
    setType(initial?.type || 'income'); setDescription(initial?.description || ''); setParty(initial?.party || '');
    setValue(initial?.value ? String(initial.value).replace('.', ',') : '');
    setDate(cleanDate(initial?.date || initial?.dueDate) || getBrazilDateString());
    setCategory(initial?.category || ''); setNotes(initial?.notes || ''); setSaving(false);
  }, [open, kind, initial]);
  if (!open) return null;
...
export const AbaFinanceiro = ({ userId, sales = [], products = [], onOpenSale, onOpenProduct, onReceiveInstallment }) => {
  ...
  const automaticMovements = useMemo(() => {
    ...
    purchaseGroups.forEach(group => {
      const hadCashOut = group.deferred ? group.financialPaid : true;
      const paymentDate = cleanDate(group.deferred ? group.paidAt : group.purchaseDate);
      const paymentDateTime = group.deferred ? group.paidAtDateTime : group.purchaseDateTime;
      if (hadCashOut && paymentDate && group.amountActuallyPaid > 0) {
        rows.push({
          id: `stock-${group.key}`,
          type: 'expense',
          date: paymentDate,
          dateTime: paymentDateTime,
          amount: group.amountActuallyPaid,
          ...
          detail: group.deferred ? `${paymentLabel(group.paymentMethod)} · paga` : paymentLabel(group.paymentMethod),
          source: 'stock',
      ...
      group.items.forEach(item => item.events.forEach((event, index) => {
        const amount = money(event.amount || num(event.quantity) * item.unitCost);
        if (event.hadCashOut && amount > 0 && event.date) rows.push({
          id: `stock-refund-${item.product.id}-${item.movement.id}-${event.id || index}`,
          type: 'income', date: cleanDate(event.date), dateTime: event.createdAt || '', amount,
          description: `Estorno de compra · ${item.product.name}`, detail: event.reason || 'Devolução ao fornecedor',
  ...
  const stockPayables = useMemo(() => purchaseGroups
    .filter(group => group.deferred)
    .map(group => ({
      id: `stock-ap-${group.key}`,
      source: 'stock',
      product: group.first.product,
      productId: group.first.product.id,
      movementId: group.first.movement.id,
      batchId: group.batchId,
      purchaseItems: group.items.map(item => ({ productId: item.product.id, movementId: item.movement.id })),
      description: group.batchId ? `Compra de mercadoria em lote · ${group.itemCount} produtos` : `Compra de mercadoria · ${group.first.product.name}`,
      party: paymentLabel(group.paymentMethod),
      dueDate: group.dueDate,
      value: group.financialPaid ? group.amountActuallyPaid : group.remainingAmount,
      paid: group.financialPaid,
      paidAt: group.paidAt,
      paidAtDateTime: group.paidAtDateTime,
      canceled: group.fullyCanceled,
      partial: group.partiallyCanceled,
      status: group.fullyCanceled
        ? statusOf(group.dueDate, false, false, true, 'Paga')
        : group.partiallyCanceled && !group.financialPaid
          ? { label: 'Parcialmente cancelada', cls: 'is-partial' }
          : statusOf(group.dueDate, group.financialPaid, false, false, 'Paga')
    })), [purchaseGroups]);
  ...
  const totals = useMemo(() => {
    ...
    return {
      income: money(income), expense: money(expense), balance: money(income - expense),
      openReceivable: money(receivables.filter(i => !i.paid && !i.canceled).reduce((sum, i) => sum + i.value, 0)),
      openPayable: money(payables.filter(i => !i.paid && !i.canceled).reduce((sum, i) => sum + i.value, 0))
    };
  ...
  const saveManual = async form => {
    ...
    if (form.kind === 'movement') {
      const nextItem = { id: form.id || makeId('mov'), type: form.type, description: form.description, value: form.value, date: form.date, dateTime: dateWithCurrentTime(form.date), category: form.category, notes: form.notes, createdAt: form.id ? (data.entries.find(i => i.id === form.id)?.createdAt || now) : now };
      ...
      const existing = form.id ? data.accounts.find(i => i.id === form.id) : null;
  ...
  const toggleStockPayable = async item => {
    ...
    targets.forEach(target => {
      ...
      if (!product) return;
      const updated = (product.movements || []).map(movement => movement.id === target.movementId ? { ...movement, financialPaid: nextPaid, financialPaidAt: nextPaid ? getBrazilDateString() : null, financialPaidAtDateTime: nextPaid ? now : null } : movement);
      batch.update(doc(db, 'artifacts', APP_ID, 'users', userId, 'products', target.productId), { movements: updated });
      writes += 1;
    });
  ...
  const originButton = item => (item.source === 'sale' || item.source === 'sale-refund') && item.sale
    ? h('button', { className: 'finance44-action', onClick: () => onOpenSale?.(item.sale) }, h(Eye, { size: 14 }), 'Ver venda')
    : (item.source === 'stock' || item.source === 'stock-refund') && item.product && !item.batchId
      ? h('button', { className: 'finance44-action', onClick: () => onOpenProduct?.(item.product) }, h(Package, { size: 14 }), 'Ver produto')
      : null;
  ...
  return h('section', { className: 'finance44 page-stack animate-fade-in' },
    ...
    loading
      ...
      : tab === 'movements'
        ? h('div', { className: 'finance44-list' }, movements.length === 0
            ? h('div', { className: 'finance44-empty' }, 'Nenhuma movimentação neste período.')
            : movements.map(item => h('div', { key: item.id, className: 'finance44-row finance44-movement-row' },
                ...
                h('div', { className: 'finance44-actions' }, item.source === 'manual'
                  ...
                  : originButton(item) || h('span', { className: 'finance44-source' }, item.batchId ? 'Lote automático' : item.source === 'manual-account' ? 'Conta manual' : 'Automático'))
              )))
        : h('div', { className: 'finance44-list' }, filteredAccounts.length === 0
            ? h('div', { className: 'finance44-empty' }, 'Nenhuma conta encontrada neste período.')
            : filteredAccounts.map(item => h('div', { key: item.id, className: `finance44-row finance44-account-row ${item.canceled ? 'is-canceled-row' : ''}` },
                h('div', { className: 'finance44-main' }, h('strong', null, item.description), h('span', null, `${item.party || (item.source === 'sale' ? 'Venda a prazo' : item.source === 'stock' ? 'Compra de mercadoria' : 'Manual')} · Venc. ${formatDate(item.dueDate)}${item.paid ? ` · Pago em ${formatDateTime(item.paidAt, item.paidAtDateTime)}` : ''}`)),
                  ...
                  item.source === 'sale'
                    ...
                    : item.source === 'stock'
                      ? h(React.Fragment, null,
                          ...
                          item.batchId ? h('span', { className: 'finance44-source' }, `${item.purchaseItems.length} produtos`) : h('button', { className: 'finance44-action', onClick: () => onOpenProduct?.(item.product) }, 'Ver produto')
                        )
                      : h(React.Fragment, null,
