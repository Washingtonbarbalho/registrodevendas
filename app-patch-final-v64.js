...
export const applyFinalPatches = source => {
  ...
  const fixedPaths = {
    modals: './modals-fixed-v59.js',
    'nova-venda': './nova-venda-fixed-v60.js',
  ...
  const freshModules = new Set([
    ...
    'aba-produtos','aba-clientes','aba-financeiro-v54','stock-movement-modal-v52',
    'aba-relatorios-v64','reports-engine-v62','aba-taxas','payment-settings','utils'
  ]);
  ...
  source = source.replace(/(['"])(\.\/[^'"]+?\.js)(?:\?[^'"]*)?\1/g, (match, quote, modulePath) => {
    ...
    if (freshModules.has(moduleName) || resolved.includes('v52') || resolved.includes('v54') || resolved.includes('v59') || resolved.includes('v60') || resolved.includes('v62') || resolved.includes('v63') || resolved.includes('v64')) url.search = '?v=64';
    return `'${url.href}'`;
  });
