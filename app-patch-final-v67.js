...
export const applyFinalPatches = source => {
  ...
  const freshModules = new Set([
    ...
    'aba-produtos','aba-produtos-v67','aba-clientes','aba-financeiro-v67','stock-movement-modal-v52','batch-stock-modal-v67',
    'aba-relatorios-v65','reports-engine-v65','sale-pdf-v65','aba-taxas','payment-settings','utils'
  ]);
  ...
  source = source.replace(/(['"])(\.\/[^'"]+?\.js)(?:\?[^'"]*)?\1/g, (match, quote, modulePath) => {
    ...
    if (freshModules.has(moduleName) || /v(52|54|59|60|65|66|67)/.test(resolved)) url.search = '?v=67';
    return `'${url.href}'`;
  });
