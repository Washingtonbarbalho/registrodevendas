import {
  ...
  AlertTriangle, ChevronRight, ChevronLeft, MoreHorizontal, LayoutGrid, X, Filter, ChevronDown
} from '../core.js';
...
const Pagination = ({ totalItems, itemsPerPage, currentPage, onPageChange }) => {
  ...
  return React.createElement('div', { className: 'flex justify-center items-center gap-2 mt-6 py-2 select-none' },
    ...
    showAllPagesModal && React.createElement('div', { className: 'fixed inset-0 bg-black/50 z-[90] flex items-center justify-center p-4' },
      React.createElement('div', { className: 'bg-white rounded-2xl w-full max-w-sm p-4 animate-fade-in shadow-2xl' },
        ...
        React.createElement('div', { className: 'grid grid-cols-5 gap-2 max-h-60 overflow-y-auto p-1' },
          Array.from({ length: totalPages }, (_, i) => i + 1).map(page => React.createElement('button', {
            ...
            className: `p-2 rounded-lg font-bold text-sm border ${currentPage === page ? 'bg-yellow-500 text-white border-yellow-500' : 'border-slate-100 text-slate-600 hover:bg-slate-50'}`
          }, page))
        )
      )
    )
    ...
    expanded && React.createElement('div', { className: 'mt-4 pt-4 border-t border-slate-50 space-y-3 animate-fade-in' },
      React.createElement('div', { className: 'flex gap-2' },
        React.createElement('button', {
          ...
          className: `flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${period === 'month' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`
        }, 'Mês Atual'),
        React.createElement('button', {
          onClick: () => onPeriodChange('custom'),
          className: `flex-1 py-2 text-xs font-bold rounded-lg transition-colors ${period === 'custom' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`
        }, 'Personalizar')
      ),
...
const ConfirmModal = ({ isOpen, onClose, onConfirm, title, message }) => {
  ...
  return React.createElement('div', { className: 'fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[90] backdrop-blur-sm' },
    React.createElement('div', { className: 'bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-fade-in text-center flex flex-col' },
      React.createElement('div', { className: 'mx-auto w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4 shrink-0' },
      ...
      React.createElement('p', { className: 'text-slate-500 mb-6 shrink-0 text-sm' }, message),
...
export { MoneyInput, Pagination, DateRangeFilter, ConfirmModal };
