...
export const applyFinalPatches = source => {
  ...
  const freshModules = new Set([
    ...
    'aba-taxas','payment-settings','utils'
  ]);
  ...
  source = source.replace(/(['"])(\.\/[^'"]+?\.js)(?:\?[^'"]*)?\1/g, (match, quote, modulePath) => {
    ...
    if (freshModules.has(moduleName) || resolved.includes('v52') || resolved.includes('v54') || resolved.includes('v59') || resolved.includes('v60')) url.search = '?v=60';
    return `'${url.href}'`;
  });
