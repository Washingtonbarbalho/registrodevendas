...
const response = await fetch(`./modals.js?v=${VERSION}`, { cache: 'no-store' });
if (!response.ok) {
    throw new Error(`Não foi possível carregar os modais (${response.status}).`);
}

let source = await response.text();

const patchComponent = (startMarker, endMarker, patcher) => {
    const start = source.indexOf(startMarker);
    const end = source.indexOf(endMarker, start + startMarker.length);
    if (start < 0 || end < 0) {
        throw new Error(`Não foi possível localizar o componente ${startMarker}.`);
    }
    const originalBlock = source.slice(start, end);
    const patchedBlock = patcher(originalBlock);
...
patchComponent(
    ...
    block => block
        .replace(
            'className: "app-modal-panel bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-fade-in flex flex-col max-h-[95vh]"',
            'className: "app-modal-panel desktop-modal desktop-modal-product-form bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-fade-in flex flex-col max-h-[95vh]"'
        )
        .replace(
            'className: "p-6 border-b border-slate-100 flex justify-between items-center shrink-0"',
            'className: "desktop-modal-header product-form-header p-6 border-b border-slate-100 flex justify-between items-center shrink-0"'
        )
        .replace(
            'className: "p-6 overflow-y-auto flex-1 space-y-4 no-scrollbar"',
            'className: "desktop-modal-body product-form-grid p-6 overflow-y-auto flex-1 space-y-4 no-scrollbar"'
        )
        .replace(
            'className: "bg-blue-50 p-3 rounded-xl border border-blue-100 flex items-start gap-2"',
            'className: "product-form-note desktop-span-full bg-blue-50 p-3 rounded-xl border border-blue-100 flex items-start gap-2"'
        )
        .replace(
            'className: "bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3"',
            'className: "product-basic-card bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3"'
        )
        .replace(
            'className: "bg-white p-4 rounded-xl border border-slate-200 space-y-3 shadow-sm"',
            'className: "product-pricing-card bg-white p-4 rounded-xl border border-slate-200 space-y-3 shadow-sm"'
        )
        .replace(
            'className: `p-4 rounded-xl border transition-colors ${isPromo ?',
            'className: `product-promo-card desktop-span-full p-4 rounded-xl border transition-colors ${isPromo ?'
        )
        .replace(
            'className: "p-6 border-t border-slate-100 flex gap-3 shrink-0 bg-white rounded-b-2xl"',
            'className: "desktop-modal-footer product-form-footer p-6 border-t border-slate-100 flex gap-3 shrink-0 bg-white rounded-b-2xl"'
        )
);
...
patchComponent(
    ...
    block => block
        .replace(
            'className: "app-modal-panel bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-fade-in flex flex-col"',
            'className: "app-modal-panel desktop-modal desktop-modal-stock-movement bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-fade-in flex flex-col"'
        )
        .replace(
            'className: "p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl"',
            'className: "desktop-modal-header stock-movement-header p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl"'
        )
        .replace(
            'className: "p-5 space-y-4"',
            'className: "desktop-modal-body stock-movement-grid p-5 space-y-4"'
        )
        .replace(
            'className: "bg-slate-100 p-3 rounded-xl border border-slate-200 flex justify-between items-center"',
            'className: "stock-product-summary desktop-span-full bg-slate-100 p-3 rounded-xl border border-slate-200 flex justify-between items-center"'
        )
        .replace(
            'className: "p-4 border-t border-slate-100 flex gap-3 bg-white rounded-b-2xl"',
            'className: "desktop-modal-footer stock-movement-footer p-4 border-t border-slate-100 flex gap-3 bg-white rounded-b-2xl"'
        )
);
...
const originalModuleUrl = URL.createObjectURL(new Blob([source], { type: 'text/javascript' }));
let originalModule;
try {
    originalModule = await import(originalModuleUrl);
} finally {
    URL.revokeObjectURL(originalModuleUrl);
}
