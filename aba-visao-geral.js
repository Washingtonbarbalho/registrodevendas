...
export const AbaVisaoGeral = ({ 
    ...
    // Filtra as transações para os modais baseado no período selecionado
    ...
    return React.createElement('div', { className: "space-y-6 animate-fade-in" },
        ...
        React.createElement('div', { className: "flex flex-col md:flex-row gap-3 bg-white p-4 rounded-xl shadow-sm border border-slate-100 justify-between items-center" },
            React.createElement('div', { className: "flex bg-slate-100 rounded-lg p-1 w-full md:w-auto" },
                [{id:'month', label:'Este Mês'}, {id:'custom', label:'Personalizado'}].map(opt => 
                    React.createElement('button', { key: opt.id, onClick: () => setDashPeriod(opt.id), className: `flex-1 md:flex-none px-4 py-2 text-xs font-bold rounded-md transition-colors ${dashPeriod === opt.id ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}` }, opt.label)
                )
            ),
            dashPeriod === 'custom' && React.createElement('div', { className: "flex items-center gap-2 w-full md:w-auto" },
                ...
                React.createElement('span', { className: "text-slate-400 font-bold" }, "até"),
        ...
        React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-3 gap-4" },
            // A Receber
            React.createElement('div', { onClick: () => setReceivablesModalOpen(true), className: "bg-white p-5 rounded-2xl border border-blue-100 shadow-sm relative overflow-hidden cursor-pointer hover:shadow-md hover:-translate-y-1 transition-all group" },
            ...
            // Entrou em Caixa
            React.createElement('div', { onClick: () => setCashInModalOpen(true), className: "bg-emerald-500 p-5 rounded-2xl shadow-lg relative overflow-hidden cursor-pointer hover:bg-emerald-600 hover:shadow-emerald-200 transition-all group text-white" },
            ...
            // Saiu do Caixa
            React.createElement('div', { onClick: () => setCashOutModalOpen(true), className: "bg-white p-5 rounded-2xl border border-red-100 shadow-sm relative overflow-hidden cursor-pointer hover:shadow-md hover:-translate-y-1 transition-all group" },
        ...
        // CARDS DE LUCRO (Inteligência Real)
        ...
        (dashboardTotals.totalOverdue > 0 || dashboardTotals.totalUpcoming > 0) && React.createElement('div', { className: "grid grid-cols-1 md:grid-cols-2 gap-4" },
            dashboardTotals.totalOverdue > 0 && React.createElement('div', { onClick: () => setInstallmentListModal({ open: true, type: 'overdue', data: dashboardTotals.overdueList }), className: "bg-red-50 p-4 rounded-xl border border-red-100 flex items-center justify-between cursor-pointer hover:bg-red-100 transition-colors" },
            ...
            dashboardTotals.totalUpcoming > 0 && React.createElement('div', { onClick: () => setInstallmentListModal({ open: true, type: 'upcoming', data: dashboardTotals.upcomingList }), className: "bg-yellow-50 p-4 rounded-xl border border-yellow-100 flex items-center justify-between cursor-pointer hover:bg-yellow-100 transition-colors" },
                React.createElement('div', { className: "flex items-center gap-3" },
        ...
        // MODAIS
        React.createElement(FinancialListModal, { isOpen: cashInModalOpen, onClose: () => setCashInModalOpen(false), title: "Entradas em Caixa", type: "income", transactions: periodIncomes, onAddManual: () => { setCashInModalOpen(false); onOpenTransactionModal('income'); } }),
        React.createElement(FinancialListModal, { isOpen: cashOutModalOpen, onClose: () => setCashOutModalOpen(false), title: "Saídas do Caixa", type: "expense", transactions: periodExpenses, onAddManual: () => { setCashOutModalOpen(false); onOpenTransactionModal('expense'); } }),
        React.createElement(InstallmentListModal, { isOpen: receivablesModalOpen, onClose: () => setReceivablesModalOpen(false), title: "Todos os Pagamentos a Receber", items: dashboardTotals.allReceivablesList, onPay: (item) => { setReceivablesModalOpen(false); handlePayFromList(item); }, onOpenWA: handleOpenWA })
    );
};
