...
export const applyFinalPatches = source => {
  ...
  const freshModules = new Set([
    ...
    'aba-produtos','aba-clientes','aba-financeiro-v54','sale-cancellation-modal-v58',
  ...
  source = source.replace(/(['"])(\.\/[^'"]+?\.js)(?:\?[^'"]*)?\1/g, (match, quote, modulePath) => {
    ...
    if (freshModules.has(moduleName) || resolved.includes('v52') || resolved.includes('v54') || resolved.includes('v55') || resolved.includes('v58')) url.search = '?v=58';
    return `'${url.href}'`;
  });
